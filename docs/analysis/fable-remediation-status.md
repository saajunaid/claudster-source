...
| F4 | Archived tasks unreachable + no unarchive; dead toggle | **DONE** | `feat/ux-correctness` — `task.unarchived` event + reducer handler, `engine.unarchive_task`, `POST /api/tasks/{id}/unarchive`, `GET /api/board?include_archived=1`; web: `useBoard(includeArchived)` (Board fetches archived only when toggle on) + drawer Unarchive button. TDD reducer/engine/api + CardDrawer (d9b2265) |
| F2 | Pipeline-preset seeds a card into a lane that doesn't exist yet | **DONE** | `feat/ux-correctness` — `applyPipelinePreset` only stages lanes + arms a `pendingSeed` ref; the explainer card is created in the save `onSuccess` (after lanes persist, guarded by the returned config containing "Ideas"), with seed failures surfaced not swallowed. Settings web TDD (75fcbda) |
| F3 | Default agent-lane config references lanes the board lacks | **DONE** | `feat/ux-correctness` — `_validate_merged_config` rejects a config where an agent-track lane present on the board auto-advances to a missing lane (the real silent-failure; both consumers swallow the error). Dormant agent lanes not checked, so the default agent-disabled board + the bootstrapping state stay valid. TDD (8802888) |
| F25 | "Report bug" promises stakeholders uploads that 403 | **DONE** | `feat/ux-correctness` — `BugForm` reads `useAuth().role`; the attachment picker + paste-to-attach render only for contributor+ (`canAttach = role !== "stakeholder"`). Bug creation stays open to all roles. Web TDD (332b0e7) |
